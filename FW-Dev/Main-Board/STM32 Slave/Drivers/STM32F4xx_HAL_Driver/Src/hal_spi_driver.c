...
void  hal_spi_configure_TImode(SPI_TypeDef *SPIx, uint32_t TIMODE)
	{
	if(TIMODE)
	{
	  SPIx->CR2 |= SPI_REG_CR2_FRF;
	}
	else
	{
		SPIx->CR2 &= ~SPI_REG_CR2_FRF;
...
void hal_spi_init(spi_handle_t *spi_handle)
{
	...
	/*Configure TI MODE*/
	hal_spi_configure_TImode(spi_handle->Instance,spi_handle->Init.TIMode);
}
