...
/*INPUT CAPTURE BEGIN*/
///////////////////////////Input Capture Variables and Function///////////////////////////
float Frequency_Send=0;
long unsigned int Frequency_Input=0;
...
//////////////////////Communication///////////////////////////
char UART_TX_Buffer[11];
char UART_TX_Buffer_Clear[11]={0};
char RX_Buffer[100];
...
int main(void)
{
    ...
    while(1)
    {
        ...
        //UART_TX_String("Microphone Testing\n");
        //ftoa(Frequency_Send, res, 1);
        //UART_TX_String(res);
        //Newline();
    }
...
__interrupt void Timer_A2_ISR(void)
{
    ...
    Counter=TA2CCR2;                   //Copying Value of Timer Compare Register Counter Value.
    Difference=Counter-Prev_Counter;//Calculating Difference
    Prev_Counter=Counter;   //Setting New Counter to previous Counter
    Frequency_Array[Frequency_Array_Count]=Difference;
...
//    Preuse_UART_TX(TX_Packet[0]);
//    Preuse_UART_TX(TX_Packet[1]);
//    Preuse_UART_TX(TX_Packet[2]);
//    Preuse_UART_TX(TX_Packet[3]);
//    Preuse_UART_TX(TX_Packet[4]);
//    Preuse_UART_TX(TX_Packet[5]);
//    Preuse_UART_TX(TX_Packet[6]);
//    Preuse_UART_TX(TX_Packet[7]);
    DMA0CTL|=DMAEN;     //DMA Channel 0 Enable for UART TX
}
